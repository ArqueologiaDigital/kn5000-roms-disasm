...
for entry in [
	...
	key, orig_name, built_name, rom_base = entry
	built_path = f"rebuilt_ROMs/{built_name}"
	...
	if len(built) > len(original):
		msg += f"ROM is too big! ({len(built)} vs {len(original)} expected)\n"
	elif len(built) < len(original):
		msg += f"ROM is too small ({len(built)} vs {len(original)} expected)\n"

	compare_len = min(len(original), len(built))
	score = 0
	first_mismatches = []
	for i in range(compare_len):
		if original[i] == built[i]:
			...
			first_mismatches.append((i, original[i], built[i]))

	percentage = f"{100*score/len(original):0.2f}%"
	badbytes = len(original) - score + max(0, len(original) - len(built))
	...
	if first_mismatches:
		...
		msg += f"  {'Address':>10s}  {'Original':>8s}  {'Built':>8s}\n"
		for addr, orig_byte, built_byte in first_mismatches:
			rom_addr = rom_base + addr
			msg += f"  0x{rom_addr:06X}  0x{orig_byte:02X}       0x{built_byte:02X}\n"
...
# Legacy ASL build comparisons (optional - only if .rebuilt.rom files exist)
asl_msg = ""
for entry in [
	...
	rebuilt_path = f"rebuilt_ROMs/{rebuilt_name}"
	if not os.path.exists(rebuilt_path):
		continue

	original = open(f"original_ROMs/{original_name}", "rb").read()
	rebuilt = open(rebuilt_path, "rb").read()

	score = sum(1 for i in range(min(len(original), len(rebuilt))) if original[i] == rebuilt[i])
	percentage = f"{100*score/len(original):0.2f}%"
	badbytes = len(original) - score

	asl_msg += f"==== ASL build ({key}) ====\n"
	if badbytes:
		asl_msg += f"Similarity: {percentage}  ({badbytes} incorrect bytes)\n\n"
	else:
		asl_msg += f"Similarity: {percentage}\n\n"
