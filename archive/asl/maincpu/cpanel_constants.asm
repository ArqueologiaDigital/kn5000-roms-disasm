...
CPANEL_STATE_MACHINE_INDEX	EQU 8D8Ah ; (byte)
CPANEL_PACKET_BYTE_COUNT	EQU 8D8Bh ; (byte) // range 0-17
				   ;          (0, 1 or 2 + nibble)
CPANEL_TX_RX_FLAGS		EQU 8D8Ch ; (8 bits)
			;   8D8Dh: unused byte?
PFCR_VALUE			EQU 8D8Eh ; (byte)
PFFC_VALUE			EQU 8D8Fh ; (byte)
			;   8D90h: unused byte?
CPANEL_UNUSED_1			EQU 8D91h ; (byte) // This one looks pointless...
CPANEL_PROTOCOL_FLAGS		EQU 8D92h ; (8 bits)
CPANEL_PANEL_DETECT_FLAGS	EQU 8D93h ; (8 bits)
...
CPANEL_COUNTER_UP_TO_20		EQU 8D98h ; (byte) counts up to 014h (=20).
			;   8D99h: unused byte?
CPANEL_COUNTER_UP_TO_42		EQU 8D9Ah ; (byte) counts up to 02ah (=42).
...
CPANEL_RX_READ_PTR		EQU 8D9Dh ; (word) NOTE: Used as index IY for
				   ;         CPANEL_RX_RING_BUFFER[IY MOD 05Ch]
				   ; in code near CPanel_RX_ProcessWithFlag and CPanel_RX_SyncPacket
CPANEL_RX_WRITE_PTR		EQU 8D9Fh ; (word)
CPANEL_RX_RING_BUFFER		EQU 8DA1h ; 05ch (=92) bytes
CPANEL_LED_READ_PTR		EQU 8DFDh ; (word)
CPANEL_LED_WRITE_PTR		EQU 8DFFh ; (word)
			;   8E00h: unused byte?
...
CPANEL_RX_EVENT_QUEUE		EQU 000200adh ; (128 bytes) Not sure yet what is placed here
...
CPANEL_LED_EVENT_QUEUE		EQU 00020137h ; (128 bytes), Also not sure yet here.
