...
SeqStep_FileBufferFinal:
	...
	lda_24	xwa, 135706
	ld	xiz, xwa
	.byte 0xbf, 0x06, 0x02, 0x00, 0x00, 0x9f, 0x06, 0x3f, 0x0a, 0x00
	jr	ge, 58
	ld	a, (xiz+22)
	and	a, 3
	cps	a, 3
	...
	.byte 0xbe, 0x16, 0xca
	jr	nz, 4
	.byte 0x8e, 0x16, 0x3c, 0x80
	incm	1, (xsp+6)
	lda	xiz, (xiz+538)
	.byte 0x9f, 0x06, 0x3f, 0x0a, 0x00
	jr	lt, -58
	ld	hl, (xsp+4)
	pop	xiz
	inc	4, xsp
...
SeqByteBlock_StyleBitmapRef:
	...
	lda	xwa, (xwa+26)
	push	xwa
	.byte 0x0b, 0x01, 0x00, 0x9f, 0x1c, 0x04, 0x9f, 0x1c, 0x04, 0x9f, 0x1c, 0x04
	ld	xwa, (xsp+54)
	push	xwa
	ld	xwa, (xsp+58)
	ld	xwa, (xwa+14)
...
SeqByteBlock_ChannelContainer:
	...
	.byte 0xb8, 0x03, 0xc9
	jr	z, 25
	.byte 0x9f, 0x14, 0x04, 0x0b, 0x00, 0x00
	ld	xwa, (xsp+20)
	push	xwa
	calr	-3924
	inc	8, xsp
	...
	jr	nz, 47
	ld	xwa, (xiz+34)
	.byte 0xb8, 0x16, 0xb3, 0x0b, 0x28, 0x00
	lds32	xwa, 0
	push	xwa
	ld	xwa, (xsp+18)
	push	xwa
	...
	ld	(xwa+22), 0
	ld	xwa, (xiz+34)
	.byte 0x98, 0x14, 0x3f, 0x23, 0x00
	jr	nz, 8
	ld	xwa, (xiz+34)
	.byte 0xb8, 0x14, 0x02, 0x00, 0x00
	pop	xiz
	lda	xsp, (xsp+12)
	ret
	dec	4, xsp
...
SeqChan_InitChannelState:
	...
	dec	6, xsp
	pushw	iz
	.byte 0xbf, 0x02, 0x02, 0x00, 0x00, 0xbf, 0x06, 0x02, 0x00, 0x00, 0x9f, 0x14, 0x3f, 0x00, 0x00
	jr	z, 12
	ld	xwa, (xsp+12)
	.byte 0xb8, 0x03, 0xbf
	ld	xwa, (xsp+12)
	...
	jr	z, 28
	.byte 0xf5, 0xe8, 0x00, 0x0a, 0xbf, 0x06, 0x02, 0x00, 0x00
	decm	1, (xsp+20)
	.byte 0x9f, 0x16, 0x3f, 0x00, 0x00
	jr	z, 95
	.byte 0xbf, 0x14, 0x02, 0x00, 0x00
	lds	iz, 1
	jr	86
	ld	xwa, (xsp+16)
	cp	(xwa), 10
...
SeqChan_ProcessEventArg1:
	.byte 0x0b, 0x01, 0x00, 0x9f, 0x0e, 0x04
	ld	xwa, (xsp+12)
	push	xwa
	ld	xwa, (xsp+12)
	push	xwa
...
SeqChan_TraverseAndProcess:
	...
	lda_24	xwa, 135706
	ld	xiz, xwa
	.byte 0xbf, 0x04, 0x02, 0x00, 0x00, 0x9f, 0x04, 0x3f, 0x0a, 0x00
	jr	ge, 49
	ld	xwa, (xsp+12)
	ld	a, (xwa+5)
	cp	a, (xiz+23)
	...
	incm	1, (xsp+4)
	.byte 0xf3, 0xf9, 0x1a, 0x02, 0x36, 0x9f, 0x04, 0x3f, 0x0a, 0x00
	jr	lt, -49
	ld	hl, (xsp+6)
	pop	xiz
	inc	4, xsp
...
SeqChan_WriteExtendedPatch:
	...
	ld	(xbc+71), xwa
	ld	(xiz+52), 229
	.byte 0xbe, 0x03, 0xbf, 0xb9, 0x03, 0xbf
	lds	hl, 0
	jr	25
	push	xiz
	calr	-4299
...
SeqStep_SectorCompareBlock:
	...
	ldw	hl, 65535
	jr	24
	.byte 0x9f, 0x0e, 0x04, 0x0b, 0x01, 0x00
	push	xiz
	calr	-96
	ld	(xsp+12), hl
	push	xiz
...
SeqChan_ByteBlockD:
	ld	xwa, (xsp+16)
	push	xwa
	.byte 0x9f, 0x12, 0x04, 0x9f, 0x12, 0x04, 0x9f, 0x10, 0x04, 0x9f, 0x14, 0x04
	ld	xwa, (xsp+16)
	ld	a, (xwa+4)
	extz	wa
	pushw	wa
	...
	ld	xiz, (xsp+14)
	ld	xbc, (xsp+10)
	.byte 0xbf, 0x04, 0x02, 0x00, 0x00, 0x91, 0x3f, 0x00, 0x00
	jr	nz, 5
	lds	hl, 0
	jrl	220
	incdi16_24	1, 141086
	...
	cp	wa, 47
	jr	nz, 125
	.byte 0xb1, 0x02, 0x1f, 0x00
	lds	hl, 0
	jrl	170
	ld	xwa, (xiz)
	.byte 0xb8, 0x02, 0xb3, 0xb1, 0x02, 0x21, 0x00
	ld	xwa, (xiz)
	push	xwa
	calr	-290
	ld	xwa, (xiz)
	...
	jr	114
	ld	xwa, (xiz)
	.byte 0xb8, 0x02, 0xb3, 0xb1, 0x02, 0x06, 0x00
	ld	xwa, (xiz)
	ld	xwa, (xwa+26)
	ld	xwa, (xiz)
	push	xwa
	...
	jr	75
	ld	xwa, (xiz)
	.byte 0xb8, 0x02, 0xb3, 0xb1, 0x02, 0x20, 0x00
	ld	xwa, (xiz)
	push	xwa
	calr	-369
	ld	xwa, (xiz)
	...
	jr	54
	ld	xwa, (xiz)
	.byte 0xb8, 0x02, 0xb3, 0xb1, 0x02, 0x24, 0x00
	ld	xwa, (xiz)
	push	xwa
	calr	-406
	ld	xwa, (xiz)
...
SeqChan_ByteBlockE:
	...
	sub	(xwa+16), iz
	ld	xwa, (xsp+22)
	.byte 0x98, 0x10, 0x3f, 0x00, 0x00
	jr	z, 102
	ld	xwa, (xsp+22)
	lda	xde, (xwa+12)
	ld	bc, iz
	ld	xwa, (xsp+2)
	.byte 0x98, 0x26, 0x41, 0xa2, 0x81
	ld	(xde), xbc
	add	(xsp+10), iz
	ld	xwa, (xsp+2)
	ld	bc, (xsp+10)
...
SndTable_ByteBlock_ReadOps:
	...
	st32_24	141102, xwa
	ld32_24	xwa, 254958
	.byte 0xb8, 0x02, 0x02, 0x00, 0x00
	ld32_24	xwa, 254958
	.byte 0xf3, 0xe1, 0x06, 0x04, 0x02, 0x00, 0x00
	ld32_24	xbc, 254958
	lds	wa, 2
	call	15671408
	ld32_24	xwa, 254958
	...
	call	15671777
	ld	xiz, xhl
	.byte 0x9e, 0x02, 0x3f, 0x00, 0x00
	jr	z, 17
	.byte 0xbe, 0x02, 0x02, 0xfe, 0xff
	ld	xwa, xiz
	ld	xbc, xwa
	lds	wa, 3
	call	15671408
	...
	cps	hl, 0
	jr	z, 21
	.byte 0xb6, 0x02, 0x00, 0x00, 0xbe, 0x02, 0x02, 0xfe, 0xff
	ld	xwa, xiz
	ld	xbc, xwa
	lds	wa, 3
	call	15671408
	jr	71
	.byte 0xb6, 0x02, 0x00, 0x04
	ld	wa, (xsp+4)
	extz	xwa
	ld32_24	xbc, 141102
	sub	xbc, xwa
